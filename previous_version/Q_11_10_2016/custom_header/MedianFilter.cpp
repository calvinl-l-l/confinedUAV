...
MedianFilter::MedianFilter(int sample_size, const int seed)
{

	//medFilterWin = max(sample_size, 3);			// number of samples in sliding median filter window - usually odd #
	if (sample_size > 3)
	{
        medFilterWin = sample_size;
	}
	else
	{
        medFilterWin = 3;
	...
	medDataPointer = sample_size >> 1;				// mid point of window
	data = (int*) calloc (sample_size, sizeof(int));			// array for data
	sizeMap = (int*) calloc (sample_size, sizeof(int));		// array for locations of data in sorted list
	locationMap = (int*) calloc (sample_size, sizeof(int));	// array for locations of history data in map list
