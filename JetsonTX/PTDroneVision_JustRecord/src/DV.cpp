...
sl::ERROR_CODE DV::startRecording(int recordStep, const std::string &fname, sl::SVO_COMPRESSION_MODE compressMode)
{
	createDir(recordpath);

	outSVOname = checkFilename(recordpath, fname+postfix, ".svo");

	errRecord = this->enableRecording(outSVOname.c_str(), compressMode);

	if(errRecord != sl::SUCCESS) {
		std::cerr << "DV - Error starting recording module:" << sl::errorCode2str(errRecord) << std::endl;
		if (errRecord == sl::ERROR_CODE_SVO_RECORDING_ERROR) std::cerr << "DV - Note : This error mostly comes from a wrong path or missing writting permissions..." << std::endl;
	}
	else {
		recordOpen = true;
		recFrame = recordStep;
		std::cerr << "DV - Info: File Created: " << outSVOname << std::endl;
	}
...
void DV::runGrab()
{
	...
	if(!this->grab(runParams)) {
		...
		if(recordOpen && frame%recFrame == 0) {
			recordState = this->record();
		}
