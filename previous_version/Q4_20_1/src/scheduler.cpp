...
void read_sensors_thread(void *p1, void *p2)
{
    ...
    while (1)
    {
        ...
        cout << quad->rcIn.ch1 << ' ' << quad->rcIn.ch2 << ' ' << quad->rcIn.ch3 << ' ' << quad->rcIn.ch4 << ' ' << quad->rcIn.ch5 << endl;
...
void start_scheduler(MSP &quad, Hokuyo_lidar &lidar)
{

    thread rc_control(RC_control_thread, &quad);
    thread read_sensors(read_sensors_thread, &lidar, &quad);
    thread sample_rx(sample_rx_thread, &quad);
