...
position_controller::position_controller()
{
	...
	_estimated_hover_throttle = 1300;
}
...
void position_controller::update_z_controller(uint16_t CH_out[8], int THR_input)
{
	...
	float pos_z_error = to_discrete(_pos_target_z - _pos_z, 5);
	...
	float current_vel_z = (_pos_z - _pos_last_z)/_dt;
    ...
    cout << "_vel_target_z: " << _vel_target_z << ' '  <<  "output: " << output << ' ' << endl;
...
{
    float pos_error_z = 90 - _pos_z; // 1m

    static float pos_error_last_z = 0;

    float output =_estimated_hover_throttle + kp_pos_z * pos_error_z;


    if (_flag_alt_mode)
    {
        CH_out[0] = range_limiter(output, 1100, 1750);
        cout << "ouptut: " << CH_out[0];
    }
}
