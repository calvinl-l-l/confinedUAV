...
void sample_rx_thread(void* p1, void* p2)
{
    ...
    const char* ardu_port = "/dev/ttySAC0";
    ...
    while (1)
    {
        ...
        while (serialDataAvail(ardu_fd))
        {
            ...
            else if (ibyte==93)
            {
                ...
                lidar->alt_floor = alt_filter.in((ardu_data[16] << 8) + ardu_data[17]);

                //cout << "lidar floor " << lidar->alt_floor << endl;
...
void read_sensors_thread(void *p1, void *p2)
{
    ...
    while (1)
    {
       ...
       // cond_lidar.notify_one();
        ...
        //cout << quad->rcIn.ch1 << ' ' << quad->rcIn.ch2 << ' ' << quad->rcIn.ch3 << ' ' << quad->rcIn.ch8 << ' ' << quad->rcIn.ch4 << endl;
